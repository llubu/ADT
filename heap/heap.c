...
int my_insert(HEAP *heap, int *data)
{
    ...
    while (cpos < 0 && (heap->compare(heap->array[cpos], heap->array[ppos])))  //child is greater then parent
    { /* Heapify */
	tmp = heap->array[ppos];
...
int my_extract(Heap *heap, int *data)
{
    ...
    cpos = 0;
    lpos = heap_leftchild(cpos);
    rpos = heap_rightchild(cpos);
	...
	if (rpos < heap_size(heap) && ((compare(heap->array[rpos], heap->array[cpos])) > 0))
	{
	    npos = rpos;
	}
